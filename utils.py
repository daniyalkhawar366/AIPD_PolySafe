...
def search_interaction(drug_a_name, drug_b_name):
    ...
    Searches via OpenFDA API.
    """
    url = f"https://api.fda.gov/drug/label.json?search=drug_interactions:{drug_a_name}&limit=1"
    try:
        response = requests.get(url, timeout=10)
        ...
        if 'results' in data and len(data['results']) > 0:
            ...
            if drug_b_name.lower() in interaction_text.lower():
